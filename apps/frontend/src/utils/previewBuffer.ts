...
class PreviewBuffer {
  private outputs: Map<number, PreviewOutput> = new Map();

  // Store the final rendered HTML output
  setOutput(projectId: number, projectName: string, finalHTML: string): void {
...
${finalHTML}`;
  ...
  getOutput(projectId: number): PreviewOutput | null {
